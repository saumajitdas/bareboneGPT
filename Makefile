...
train-ci:
	PYTHONPATH=src python -m barebonegpt.train --config configs/ci.json

gen:
	PYTHONPATH=src python -m barebonegpt.generate --checkpoint checkpoints/model.pt --prompt "Hello"

serve:
	PYTHONPATH=src python -m barebonegpt.server --checkpoint checkpoints/model.pt --host 0.0.0.0 --port 8000
